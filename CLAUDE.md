...
`scaler` is a Rust CLI (edition 2024) that runs an existing host command under normalized CPU/memory flags with a deterministic capability report and a compact monitor-oriented run loop. V1 targets Linux (strong enforcement via `systemd-run` + cgroup v2) and macOS (best-effort via `taskpolicy` / `renice`). See `README.md` for the user-facing CLI surface and `docs/superpowers/specs/2026-04-02-scaler-v1-design.md` for the authoritative design contract.
...
- **`PlainFallbackBackend` is what `lib.rs` actually wires in for `scaler run` today** even on Linux/macOS. The `linux_systemd` and `macos_taskpolicy` modules currently provide capability detection (`detect_host_capabilities`) and argv builders for tests; they are not yet plugged into `execute()`. Keep this in mind when adding behavior — changes to `PlainFallbackBackend` affect every host until the platform backends are wired in.
...
`Renderer` trait with two implementations: `plain::PlainRenderer` (line-oriented streaming) and `tui::TuiRenderer` (ratatui/crossterm full-screen with a compact mode for PTY children). Both consume `MonitorSnapshot` and `OutputFrame` and never touch process state. `format_bytes` / `format_elapsed` in `ui::mod` are the canonical formatters — reuse them rather than reimplementing.
...
`docs/superpowers/specs/2026-04-02-scaler-v1-design.md` is the authoritative behavioral contract for v1 (CLI grammar, capability vocabulary, lifecycle, doctor output, signal escalation, monitor fallback). `docs/superpowers/plans/2026-04-02-scaler-v1.md` is the implementation plan it was built from. When the design doc and code disagree, the design doc wins for v1 — fix the code or escalate.
